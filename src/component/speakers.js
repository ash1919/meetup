...
import "react-toastify/dist/ReactToastify.css";
 ...
 const ListOfSpeakers=() =>{
  ...
  const getSpeakerDetails = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        setFilterSpeaker(response.data.speakers);
  ...
  const filter = async (value) => {
    const searcPath = `${url.endPoint}/speakers/?skip=0&limit=100&search=${value}`;
    const res = await Axios.get(searcPath);
    
    try {
      if (res.status === 200) {
        setFilterSpeaker(res.data);
      }
    } catch (error) {
                ...
                filterSpeaker.map((filterSpeakers) => (
                  ...
                  >
                    ...
                    <div className="flex space-x-3 pl-0 items-center mb-12">
                      ...
                      <a href={filterSpeakers.social.facebook}>
                        <img
